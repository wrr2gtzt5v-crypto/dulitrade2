...
def validate_and_filter_signal(result, market_ctx=None, trade_type="swing"):
    ...
    # שערים לפי סוג עסקה — Day Trade סף נמוך יותר כדי לאפשר יותר עסקאות
    ...
    # Confluence — Day Trade מינימום 4 (5-6 = עסקה תקינה)
    conf_score = result.get("confluence_score", 0) or 0
    if is_day and conf_score < 4:
        reasons.append(f"Confluence={conf_score} מתחת ל-4 ל-Day Trade — Setup חסר גורמים מאשרים")
    elif not is_day and conf_score < 6:
        reasons.append(f"Confluence={conf_score} מתחת ל-6 ל-Swing")
...
כוונון ציון Confidence — אל תיתן 5-6 כברירת מחדל, הכרע לכיוון אחד:
9-10: Setup קלאסי מושלם — כניסה מיידית, כל הפקטורים מאשרים
7-8:  Setup טוב — 3-4 פקטורים מאשרים, כנס בגודל מלא
5-6:  Setup קיים אבל יש ספק — כנס בגודל מצומצם (50%)
3-4:  ספק רב — המתן לאישור נוסף לפני כניסה
1-2:  אל תיכנס — לא ברור / נגד מגמה
...
3. Confluence Score: מתחת ל-4 ב-DAY_TRADE → NEUTRAL. 4-5 = מספיק לכניסה עם pattern ברור
4. אין אישור נפח → גורם מפחית ציון (לא NEUTRAL חובה), אלא אם נפח נמוך מאוד
5. אם הגרף לא ברור ואין שום דפוס מזוהה → NEUTRAL. אבל pattern בינוני + תמיכה/התנגדות = כנס
6. השתמש בכל הגרף להבנת הקשר ומגמה, אבל קבל החלטת כניסה לפי הנרות האחרונים בצד ימין
7. עדיף לסחור עם המגמה: LONG במגמת עלייה (HH/HL), SHORT במגמת ירידה (LH/LL)
8. היפוך מגמה אפשרי רק כשיש CHoCH ברור + אישור נפח + דפוס נרות — אחרת NEUTRAL
9. Entry Timing: המתן לסגירת נר מעל/מתחת הרמה לפני כניסה — מונע כניסות על פריצות מזויפות
...
סטאפים שחייבים לקבל איתות LONG/SHORT (גם אם תנאים לא מושלמים):
— Bounce ברור מתמיכה עם wick דחייה + נר סגירה חיובי → LONG, Confidence 5+, R/R 1.2+
— Pullback ל-VWAP/EMA עם נר היפוך + נפח גדל → LONG/SHORT, Confidence 5+
— Breakout מ-Consolidation עם נפח פי 1.5+ → כיוון הפריצה, Confidence 6+
— Gap Continuation: מניה שגפה מעלה + ממשיכה אחרי 30 דקות → LONG, Confidence 5+
— Mean Reversion קיצוני: RSI<25 או RSI>75 + wick דחייה → Confidence 5+
עבור סטאפים אלו: Confidence 5+ מספיק, R/R 1.2+ מספיק לכניסה

כניסות תקפות ל-Day Trade:
...
- win_probability_reason: משפט קצר מדוע הנסתברות היא X% — מה התנאים שמחזקים/מחלישים.
...
- skip_if: תנאי ספציפי שיבטל את ה-Setup (למשל: "אם נופל מתחת ל-$X"). אם אין — ריק "".
"""
